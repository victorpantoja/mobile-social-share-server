...
class UserHandlerTestCase(AsyncHTTPTestCase):
    ...
    def test_create_user(self):
        self.http_client.fetch(self.get_url('/login/create') + '?username=should-be-username&email=should-be-email@mss.com&firstName=test_create_user&lastName=should-be-last-name&gender=M' , self.stop)
    ...
    def test_create_existent_user(self):
        ...
        user = create_user(username="test_create_existent_user", email="test_create_existent_user@mss.com")

        self.http_client.fetch(self.get_url('/login/create') + '?username=%s&email=%s&firstName=%s&lastName=%s&gender=M' % (user.username, user.email, user.first_name, user.last_name) , self.stop)
