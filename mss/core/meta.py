...
def get_engine(writable=False, **kw):
    if writable:
        ...
        if not __engine_writable__:
            __engine_writable__ = create_engine(settings.DATABASE_ENGINE['write'], pool_size=settings.DATABASE_POOL_SIZE, pool_recycle=300, proxy=TimerProxy(), **kw)
        return __engine_writable__
    else:
        global __engine_readable__
        if not __engine_readable__:
            __engine_readable__ = create_engine(settings.DATABASE_ENGINE['read'], pool_size=settings.DATABASE_POOL_SIZE, pool_recycle=300, proxy=TimerProxy(), **kw)
...
def expire_session():
    ...
    __engine_writable__ = get_engine(writable=True, max_overflow=10)
